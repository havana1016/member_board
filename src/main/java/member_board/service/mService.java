...
public class mService {
    ...
    String fname = mem.getBf().getOriginalFilename();//2
    fname =System.currentTimeMillis()+"-"+fname;//2.1
        mem.setFname(fname);//3
    String savePath ="c:\\spring_img\\"+fname;//4

        if(!mem.getBf().isEmpty()){
            mem.getBf().transferTo(new File(savePath));
